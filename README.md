...
Создан по описанию «Задача №8». **API Recsflow, реальный каталог и история пользователей не предоставлены.** Поэтому используется заменяемый `DemoProvider` и воспроизводимый синтетический каталог из 3000 объектов (seed=42, распределения откалиброваны по MovieLens-100k — см. `src/recagent/catalog/calibration.py`). Названия, атрибуты, оценки и демо-история вымышлены. Прототип не подключён к Recsflow.
...
7. `Сериал похожий на «Протокол ночного поезда»` — название возьмите из каталога (кнопка «Посмотреть каталог» в сайдбаре). Поддержан осторожный поиск единственного близкого названия при небольшой опечатке или изменении окончания.
...
[Исходный результат от 14.09](report/baseline-dev.md): 39,5% успешных финальных ответов в rules-режиме на 200 dev-сценариях. В отчёте разобраны ошибки и ограничения сравнения.
