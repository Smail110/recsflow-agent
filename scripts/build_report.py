...
def main():
    ...
    rows = []
    for label, field in [("Сценарии", "scenarios"), ("Success rate, доля", "success_rate"), ("Уточнений на сценарий", "mean_clarifications"), ("Проверенных утверждений", "claim_count"), ("Доля неподтверждённых утверждений", "unsupported_claim_rate"), ("p50 обработки, мс", "latency_p50_ms"), ("p95 обработки, мс", "latency_p95_ms"), ("Попытки LLM агента", "agent_llm_calls"), ("Токены агента", "agent_tokens"), ("Ходы с fallback", "fallback_turns"), ("Завершённые оценки LLM-судьи", "llm_judge_completed"), ("Success rate LLM-судьи", "llm_judge_success_rate"), ("Токены симулятора и судьи", "evaluator_tokens"), ("Денежная стоимость", "monetary_cost")]:
        rows.append(f"<tr><td>{label}</td><td>{format_value(rules['metrics'].get(field)) if rules else 'нет прогона'}</td><td>{format_value(llm['metrics'].get(field)) if llm else 'нет прогона'}</td></tr>")
    scenario_rows = []
    if llm:
        for record in llm["records"]:
            judge = record.get("llm_judge")
            scenario_rows.append(f"<tr><td>{html.escape(record['id'])}</td><td>{'Пройден' if record['success'] else 'Не пройден'}</td><td>{'Успех' if judge and judge['success'] else 'Неуспех' if judge else 'Нет оценки'}</td><td>{html.escape(record['response']['mode'])}</td></tr>")
    load_rows = "".join(f"<tr><td>{label}</td><td>{format_value(load_test.get(key)) if load_test else 'не измерено'}</td></tr>" for label, key in [("Запросы", "requests"), ("Одновременные клиенты", "concurrency"), ("Успешные ответы", "successes"), ("Ошибки", "errors"), ("p50, мс", "p50_ms"), ("p95, мс", "p95_ms"), ("Запросов/сек", "requests_per_second")])
...
<p class="note"><b>Исходные данные:</b> предоставлено только описание задания. Каталог, история и контракт Recsflow отсутствуют. Использованы 84 вымышленных объекта и синтетическая история демо-профиля, генератор с seed=42. Этот отчёт не подтверждает качество платформы Recsflow.</p>
