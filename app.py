...
with st.sidebar:
    ...
    st.divider()
    st.selectbox("Режим подбора", ["rules", "ollama"], format_func=lambda x: "Демо · без модели" if x == "rules" else "Локальная LLM · Ollama", key="mode", on_change=new_session)
    st.selectbox("Профиль", ["demo", "new-user"], format_func=lambda x: "Демо · есть история" if x == "demo" else "Новый пользователь", key="profile", on_change=new_session)
    ...
    st.markdown('<div class="note">Демонстрационный стенд<br><b>84 вымышленных объекта</b><br>Фильмы, сериалы и курсы. История демо-профиля тоже синтетическая.</div>', unsafe_allow_html=True)
    with st.expander("Посмотреть каталог"):
        st.dataframe([{"Название": i.title, "Жанр": i.genre, "Формат": i.kind} for i in st.session_state.agent.provider.items.values()], hide_index=True)

st.markdown('<div class="eyebrow">RECAGENT / ПРОТОТИП 01</div>', unsafe_allow_html=True)
st.title("Найдём то, что вам близко.")
st.markdown('<p class="intro">Расскажите о настроении, планах или интересах.<br>Я учту пожелания и объясню каждый выбор.</p>', unsafe_allow_html=True)
...
if not st.session_state.messages:
    ...
    for column, (label, prompt) in zip(st.columns(3), examples):
        with column:
...
def render_response(response, message_index):
    ...
    for rank, recommendation in enumerate(response.recommendations, 1):
        ...
        with st.container(border=True):
            ...
            st.caption(f"{ {'series':'Сериал', 'film':'Фильм', 'course':'Курс'}[item.kind] } · {item.genre.capitalize()} · Вымышленный объект")
            st.write(recommendation.explanation)
            columns = st.columns([1, 1, 1, 2])
            for column, (label, reaction) in zip(columns, [("♡ Нравится", "like"), ("Не подходит", "dislike"), ("Уже знакомо", "seen")]):
            ...
            with st.expander("На чём основан выбор"):
                st.dataframe([{"Поле": e.field, "Значение": str(e.value), "Связь": e.relation, "Источник": e.source_item_id or e.item_id} for e in recommendation.evidence], hide_index=True, use_container_width=True)
    with st.expander("Диагностика запроса"):
        st.caption(f"{response.mode} · {response.latency_ms:.0f} мс · LLM-вызовов: {response.llm_calls_total}/8 · токенов за ход: {response.llm_tokens}")
        st.json(response.query.model_dump(exclude_none=True))
        st.caption(" → ".join(response.trace))
    st.download_button("Скачать ответ JSON", response.model_dump_json(indent=2), file_name="recagent-response.json", mime="application/json", key=f"download-{message_index}")
...
if prompt:
    ...
    with st.spinner("Подбираю варианты и проверяю факты…"):
        try:
            result = st.session_state.agent.chat(ChatRequest(message=prompt, session_id=st.session_state.sid, user_id=st.session_state.get("profile", "demo")))
